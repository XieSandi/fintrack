...
js/db.js              repository: CRUD generik (+hook efek debt/asset/attachment), seeding kategori,
                       snapshot bulanan, export/import backup, bulkDelete()/previewBulkDelete()
                       (reset data, js/views/danger.js), deleteAssetKeepHistory(), attachments
                       (foto bukti: addAttachment/getAttachment/removeAttachment)
...
js/views/             home, transactions, budget, wealth, settings, accounts, categories, goals,
                       recurring, danger
tests/precache.test.mjs cek semua file runtime ada di PRECACHE sw.js (node tests/precache.test.mjs)
...
Routing: hash (`#/home`). Nav: Home · History (transactions) · Assets (wealth: sumtab Total ·
Assets · Liquid · Debt · Claim [piutang]) · Setting.
...
- `accounts` — kantong uang (bank/ewallet/cash/rdn/broker/**credit**), currency IDR/USD,
  ...
  **`accountNumber`** (string opsional, no. rekening / no. kartu) — **SENGAJA cuma hidup di
  ...
  History. Saldo aktual BOLEH MINUS (rekening overdraft, dll) — `attachThousands(input,
  {allowNegative:true})` (utils.js, default-nya tetap strip minus buat input nominal transaksi)
  + tombol ± di samping input (keyboard numerik HP sering ga punya "-"). Akun tipe `credit`
  BEDA PRESENTASI di sheet ini (pola sama semua tampilan CC lain: "Terpakai", bukan saldo signed):
  input-nya = TAGIHAN TERPAKAI aktual (angka positif = utang), internal dibalik jadi saldo negatif
  (`parseActual = -parseTyped`), minus di sini = kelebihan bayar/saldo plus. Transaksi adjustment
  yang dibikin TETAP generic (expense/income sebesar selisih), ga ada kode khusus di jalur tulis.
...
- `transactions` — {date, time?, month:"YYYY-MM", amount, type: expense|income|transfer, accountId,
  toAccountId?, toAmount?, fxRate?, toGoalId?, fromGoalId?, categoryId, debtId?, debtDir?, assetId?,
  assetDir?, assetQty?, assetPrice?, assetDeleted?, assetSnapshot?, feeOfTxId?, attachmentId?,
  linkUrl?, note}. Transfer = 1 record, BUKAN expense. Field opsional yang lebih baru: `toAmount`/
  `fxRate` (transfer lintas mata uang, lihat bawah), `debtDir` ("borrow" = dana pinjaman masuk,
  bullet `debts`), `assetDeleted`/`assetSnapshot` (asset-nya udah dihapus tapi transaksi tetap,
  bullet `assets`), `attachmentId`/`linkUrl` (foto/link bukti, bullet `assets` tipe `receivable` &
  `attachments`) — semuanya additive, schemaVersion TIDAK naik.
  ...
  **`DEFAULT_TX_TIME` = "00:01"** (utils.js) — dianggap paling awal hari itu, BUKAN "sekarang",
  ...
  asli).
  **Transfer lintas mata uang** (`toAmount`, `fxRate` — opsional/additive, schemaVersion TIDAK
  naik) — transfer akun-ke-akun IDR↔USD: `amount` TETAP currency akun SUMBER, `toAmount` = nominal
  yang DITERIMA akun tujuan dalam currency TUJUAN, `fxRate` SELALU "1 USD = X IDR" apapun arahnya
  (sama kayak konsep kurs di seluruh app; IDR→USD bagi, USD→IDR kali). `accountBalances()` (calc.js)
  ngredit tujuan pakai `toAmount` KALAU ada (`!= null`), fallback `amount` kalau ga ada (transfer
  se-currency, data lama pra-fitur) — jadi transaksi lama ga berubah perilaku. UI-nya di
  `openTxSheet()` (tx-sheet.js): field Kurs + Diterima CUMA muncul kalau currency asal ≠ tujuan,
  default kurs `effectiveRate()`, dua field saling ngitung (yang terakhir diedit menang), disimpan
  `null` kalau se-currency (biar field lama ke-clear pas edit). Recurring transfer lintas currency
  (`recurring-sheet.js` "Catat Semua") ngitung `toAmount` pakai `effectiveRate()` SAAT posting.
  `openPayCreditSheet()` (Bayar Tagihan CC) SENGAJA batasin sumber ke akun SE-CURRENCY sama
  kartunya (ga punya field kurs) — lintas currency pakai Transfer biasa. `txRow()` nampilin
  `(+X USD)` di baris akun tujuan kalau ada `toAmount`. Cuma berlaku transfer akun-ke-akun —
  topup goal/beli asset (di bawah) tetap satu `amount` (goal/asset dihitung pakai currency akun
  sumber-nya, ga ada "akun tujuan" yang beda currency).
...
- `assets` — saham IDX (quantity dalam **LOT**, ×100 lembar saat hitung nilai), US fractional shares,
  ...
  **Hapus asset yang punya transaksi** — BOLEH, asal nilai asset-nya udah 0 (posisi habis dijual /
  piutang lunas / bond redeemed; nilai > 0 diblok dengan toast — ngilangin asset yang masih ada
  nilainya = net worth berubah tanpa jurnal). Lewat `deleteAssetKeepHistory(assetId)` (db.js,
  raw `writeBatch`, pola jalur massal sama `importAll()`/`bulkDelete()`): transaksi ber-`assetId`
  **TIDAK ikut dihapus** (riwayat cashflow di History & saldo akun tetap utuh), cuma ditandai
  `assetDeleted:true` + `assetSnapshot` ({symbol, name, type, currency, qtyless, debtorName}),
  `assetId`-nya TETAP. `home.js` `assetForTx(t)` balikin pseudo-asset dari snapshot itu kalau
  dokumen asset-nya udah ga ada → `txRow()` tetap nampilin "Beli/Jual: BBCA" + badge "asset
  dihapus", `openTxDetail()` tetap route ke sheet detail read-only (hapus doang) — reversal
  qty-nya no-op karena asset ga ketemu (`applyAssetQtyEffect()` return awal). `integrity.js`
  TIDAK nge-flag "asset ga ketemu" buat transaksi ber-`assetDeleted` (disengaja, beda dari orphan
  hapus manual di Firestore console) dan pakai `assetSnapshot.qtyless` buat validasi field.
  Asset TANPA transaksi tetap `remove()` biasa. Guard link goal (`linkedAssetIds`) TETAP berlaku
  (lepas dulu). GA ADA fitur arsip asset (sempat dibikin, dicabut — owner mau asset kosong beneran
  ilang dari list, bukan disembunyiin; riwayat: `DECISIONS.md`). Toast abis jual habis / piutang
  lunas ngingetin "bisa dihapus di Edit Asset".
  ...
  **Tipe `receivable`** (🤝 Piutang — uang yang dipinjemin ke orang, "asset mengendap"; **di UI
  SEMUA teks-nya "Claim"** (tab, sheet, toast, laporan .md) atas permintaan owner — nama
  field/fungsi/komentar di kode & dokumen ini tetap "receivable"/"piutang", jangan di-rename) — tipe
  asset BARU (bukan collection baru) yang **REUSE MESIN `qtyless`** (lihat bullet `qtyless` di
  bawah): `qtyless` DIPAKSA true di form (checkbox-nya di-hide), `quantity` selalu 1,
  `manualPrice` = **SISA PIUTANG** (nilai asset ini, `receivableLocalValue()`/`receivableValueIDR()`
  calc.js, clamp ≥0), `avgBuyPrice` = **TOTAL YANG PERNAH DIPINJAMKAN** (informatif; kalau kosong
  pas dibuat dari form, default = sisa). Field tambahan: `debtorName` (siapa, WAJIB), `dueDate`
  ("YYYY-MM-DD", kapan bisa ditagih, opsional) — `null` kecuali tipe receivable (pola bond/capex).
  `symbol` di-hide (kayak capex), `name` = "Nama Piutang". **Piutang GA BISA DIHAPUS — cuma
  DIARSIPKAN** (`isArchived`, KHUSUS tipe ini; tombol Hapus ga dirender di Edit Asset buat
  receivable, diganti checkbox "📦 Arsipkan piutang"): `receivableLocalValue()` → 0 kalau arsip
  (berhenti dihitung net worth/total, pola sama bond `redeemed` — jadi arsip juga jalan buat
  write-off piutang yang ga ketagih), di-exclude dari snapshot breakdown & report, ditaruh di
  section "📦 Arsip" paling bawah tab Piutang (klik → detail, un-archive lewat Edit; tombol Terima
  Pembayaran/Kasih Pinjaman disembunyiin buat yang arsip). **`linkUrl`** (transaksi, additive):
  link bukti opsional di sheet Piutang Baru / Kasih Pinjaman / Terima Pembayaran + bisa nyusul
  dari detail transaksi (`renderTxLink()`), dinormalisasi `normalizeLink()` (tanpa skema →
  https://, cuma http/https), ditampilin sebagai anchor `target=_blank rel=noopener`, badge 🔗 di
  History & riwayat. **Punya TAB SENDIRI di Wealth: "Piutang"
  (sumtab kelima, samping Debt — `renderReceivables()`, `groupTab === "receivable"`, `.sumtabs`
  jadi 5 kolom)** — list per peminjam (badge TAGIH/Lunas, urut: udah bisa ditagih → tanggal tagih
  terdekat → lunas paling bawah) + tombol Terima Pembayaran / Kasih Pinjaman / Edit + "＋ Tambah
  Piutang" → **`openNewReceivableSheet()`**: nominal + siapa + keterangan + **Dari Akun (sumber
  dana, WAJIB)** + tanggal/jam + tgl tagih + catatan + foto — SATU aksi bikin dokumen asset
  `receivable` DAN transaksi transfer `assetDir:"buy"` (akun sumber kepotong), currency ikut akun
  sumber. Klik item → **`openReceivableDetailSheet(a)`**: progress dibayar/total, sisa, tgl tagih,
  **riwayat semua transaksi** piutang itu (pinjaman keluar / pembayaran — cicilan = banyak transaksi
  `sell`, satu per pembayaran, masing-masing punya tanggal/nominal/akun/catatan/foto sendiri), klik
  baris → detail transaksi read-only. **Foto bukti** (opsional, cuma jalur piutang): input file di
  sheet Piutang Baru / Kasih Pinjaman / Terima Pembayaran + bisa nyusul dari detail transaksi;
  dikompres client-side `compressImage()` (utils.js, data URL JPEG ≤ ~450KB — diukur dari panjang
  data URL, bukan byte mentah; jangan dinaikin mepet 1 MiB, foto ikut cache offline Firestore) → collection
  **`attachments`** (db.js `addAttachment()`/`getAttachment()`, doc {txId, mime, data (data URL)}),
  transaksi nyimpen `attachmentId` (additive). BUKAN Firebase Storage & BUKAN field di transaksi
  (alasan: `DECISIONS.md`); foto di-fetch on demand pas detail dibuka, ikut kehapus di `remove()`
  hook & `bulkDelete()`, ikut backup (`COLLECTIONS`). Tampilan History (`txRow()`): pembayaran
  piutang ditampilin kayak income (+ hijau), pinjaman keluar kayak expense (− merah), badge 📎
  kalau ada foto — TAMPILAN doang, tipe datanya TETAP `transfer` (ga masuk Income/Expense
  `monthSummary()`: uang balik dari piutang bukan pendapatan, pinjaman keluar bukan pengeluaran). Tab **Assets** (`renderAssets()`) & angka sumtab "Assets" SENGAJA EXCLUDE
  piutang (biar ga dobel kebaca) — `totalAssetsIDR()` sendiri TETAP termasuk (konsep net worth,
  breakdown Total misahin baris "🤝 Piutang" sendiri). Transaksinya lewat
  `openQtylessTradeSheet()` (wealth.js) yang di-relabel via object `L`: **"🤝 Kasih Pinjaman"** =
  `assetDir:"buy"` (akun didebit, sisa & total dipinjamkan naik), **"💵 Terima Pembayaran"** =
  `assetDir:"sell"` (akun dikredit, sisa turun, total dipinjamkan TETAP; guard "ga bisa terima
  lebih dari sisa"). JANGAN bikin sheet/jalur tulis terpisah buat piutang — reversal hapus
  transaksi (db.js `applyAssetQtyEffect()` cabang qtyless), `bulkDelete()`, dan validasi
  `integrity.js` OTOMATIS jalan karena `qtyless:true`. Yang BEDA dari qtyless biasa: (1)
  `assetCostIDR()` = value → **P&L SELALU 0** (piutang bukan investasi; `assetRow()` nampilin
  "sisa piutang"/"lunas" bukan P&L, report section 6 kolom P&L "—"; kalau beneran ga ketagih,
  user kurangi sisa manual via Edit Asset atau catat expense), (2) **toggle net worth**
  `settings.includeReceivablesInNetWorth` (checkbox "🤝 Sertakan Piutang" di Wealth → Total, di
  bawah toggle CAPEX, cuma muncul kalau ada ≥1 asset receivable) — **default TRUE/include**
  (`includeReceivablesSetting()`: cuma `=== false` yang exclude), BEDA dari CAPEX yang default
  exclude: uang yang dipinjemin beneran keluar dari cash, kalau ga dihitung net worth bakal
  keliatan "jatuh" persis sebesar pinjaman padahal cuma pindah bentuk (alasan lengkap:
  `DECISIONS.md`). Formula: `netWorthFromParts(parts, includeCapex, includeReceivables = true)` —
  `receivables` pola PERSIS `capex` (udah termasuk di `assets` raw, di-SUBTRACT kalau toggle OFF),
  `totalAssetsIDR()` TETAP termasuk piutang. `snapshotNetWorth()`/`netWorthComposition()` juga
  nerima param ke-3/ke-4 `includeReceivables` (default true); di chart Tren Net Worth (dua garis
  CAPEX) & Proyeksi & report-md.js piutang ngikut toggle SEKARANG buat SEMUA garis/angka (bukan
  nambah garis/kolom lagi — yang dibandingin eksplisit tetap cuma CAPEX), report section 1 & 6
  nyebut perlakuannya + rincian per peminjam. Breakdown Total tab Wealth: "📈 Assets" =
  `assetsRaw − capex − receivables` (SELALU exclude dua-duanya), baris "🤝 Piutang" terpisah CUMA
  kalau toggle ON — rows tetap sum persis ke net worth. Snapshot: `totalReceivables` top-level +
  `debtorName`/`dueDate` di `breakdown.assets` (additive, snapshot lama → 0/null, **schemaVersion
  TIDAK naik**). Sisa 0 = "✅ Lunas" (badge, pola `debts`), asset-nya boleh dihapus (transaksi
  tetap ada, lihat "Hapus asset yang punya transaksi" di atas; toast abis Terima Pembayaran yang
  bikin sisa 0 ngingetin). `dueDate` lewat & sisa > 0 → "⚠️ Udah bisa
  ditagih" di `assetRow()` + info `integrity.js` (plus: `qtyless` bukan true, sisa > total
  dipinjamkan). TIDAK auto-refresh (bukan `AUTO_TYPES`), di-exclude dari dropdown DCA recurring.
  `txRow()` (home.js) label "Pinjamkan: {debtor}" / "Pembayaran piutang: {debtor}", icon 🤝.
...
- `debts` — outstanding, monthlyInstalment, dueDay, remainingMonths, `isArchived`. Mengurangi net worth.
  **v2 (2026-09, pola SAMA PERSIS piutang — lihat bullet `assets` tipe `receivable`):** hutang
  **GA BISA DIHAPUS, cuma diarsipkan** (`isArchived`; tombol Hapus dicabut dari `openDebtSheet()`,
  diganti checkbox) — `debtOutstanding(d)` (calc.js) → 0 kalau arsip (berhenti dihitung
  `totalDebtIDR()`, jadi arsip juga = "dianggap selesai" buat sisa yang ga dibayar),
  `activeDebts()` filter list tab Debt/snapshot breakdown/report/dropdown "Potong hutang?"
  (tx-sheet & recurring, kecuali yang lagi dipakai), `brokenReason()` recurring nge-flag debt
  arsip. Tab Debt: klik item → **`openDebtDetailSheet(d)`** (progress dibayar/total, riwayat
  semua transaksi ber-`debtId`, tombol Bayar Cicilan / Tambah Pinjaman / Edit; hutang arsip di
  section "📦 Arsip" paling bawah). **"＋ Tambah Hutang" → `openNewDebtSheet()`**: nominal + nama +
  cicilan/bln + sisa bulan + tgl jatuh tempo + checkbox "💵 Dana pinjaman masuk ke akun" (default
  ON — cash loan; OFF buat BNPL/cicilan barang yang uangnya ga lewat rekening) + Ke Akun +
  tanggal/jam + catatan + foto + link → debt dibuat dengan `totalOutstanding` = nominal LANGSUNG,
  transaksi borrow-nya ditulis `add(..., {skipDebtEffect:true})` (bypass eksplisit hook, lihat
  db.js `add()` — kalau hook jalan outstanding dobel). **Arah transaksi ber-`debtId`:**
  `debtDir:"borrow"` = transfer, dana pinjaman MASUK (`accountBalances()` ngredit `accountId`,
  BUKAN income — ga masuk `monthSummary()`), outstanding NAIK, `remainingMonths` ga disentuh;
  `debtDir` kosong = pembayaran (expense biasa, perilaku lama, outstanding turun + sisa bulan −1).
  SATU sumber arah: `isDebtBorrow(t)` / `debtTxDelta(t)` (calc.js, di-test) dipakai hook
  `add()`/`handleDebtPatch()`/`remove()` DAN agregasi `bulkDelete()` — jangan hitung sign manual
  lagi. **"💵 Bayar Cicilan"** (`openDebtPaySheet()`): expense ber-`debtId` + kategori (default
  `cat_cicilan`) + foto/link bukti — jalur lama lewat `openTxSheet()` "Potong hutang?" TETAP jalan
  (tanpa foto). **"➕ Tambah Pinjaman"** (`openDebtBorrowSheet()`, di-export — dipakai `home.js`
  `openTxDetail()` buat routing transaksi borrow ke detail read-only + Hapus, JANGAN ke
  `openTxSheet()` generik yang ga ngerti `debtDir`). `txRow()`: borrow tampil "Pinjaman masuk:
  {debt}" + hijau (tampilan doang). `openTxSheet()` nampilin foto/link read-only kalau transaksi
  existing punya `attachmentId`/`linkUrl`. Cicilan = banyak transaksi pembayaran (udah dari dulu).
  ...
  `totalOutstanding` ≤ 0 → badge "Lunas 🎉" (tab Debt Wealth), bukan auto-delete — beresinnya
  lewat arsip (lihat v2 di atas), hutang ga pernah dihapus.
...
- `attachments/{id}` — {txId, mime, data (data URL JPEG hasil kompres), createdAt}. Foto bukti
  transaksi piutang (lihat bullet `assets` tipe `receivable`). TIDAK di-listen ke `state`
  (di-`getDoc` on demand), ikut backup/restore & bulk delete.
- `snapshots/{YYYY-MM}` — net worth bulanan, di-upsert otomatis saat app dibuka (`upsertSnapshot`,
  db.js). Selain total (`totalCash`/`totalAssets`/`totalCapex`/`totalReceivables`/
  `totalGoalSavings`/`totalDebt`/`netWorth` — `totalCapex` field baru sejak fitur CAPEX,
  `totalReceivables` sejak fitur piutang, snapshot lama ga punya ini → fallback 0, lihat
  bullet `assets` tipe `capex`/`receivable` & "Backfill CAPEX ke Snapshot Lama" di Known Quirks), nyimpen
  ...
  `capex`, PLUS `debtorName`/`dueDate` — `null` kecuali tipe `receivable`}), `debts` ({name, outstanding, monthlyInstalment, remainingMonths, dueDay}), `goals`
...
- `settings/main` — targetNetWorth (= **Main Milestone**, dipakai card Total Balance Home DAN
  ...
  `includeCapexInNetWorth` (default false) & `includeReceivablesInNetWorth` (default TRUE —
  lihat bullet `assets` tipe `receivable`; dua-duanya checkbox di Wealth → Total, bukan Setting),
...
`assets` tipe `capex`; − totalReceivablesIDR kalau toggle piutang di-OFF-in, default ON, lihat
bullet `assets` tipe `receivable`) + totalGoalSavingsIDR − totalDebtIDR (USD dikonversi
`effectiveRate()`).
...
1. **Setiap deploy perubahan file apapun: naikin `CACHE_VERSION` di `sw.js`** (v4 → v5 dst).
   ...
   cek pakai `node tests/precache.test.mjs` (bandingin isi `js/`, `css/`, `icons/`, index,
   manifest vs array-nya; bump versi-nya sendiri tetap manual).
...
- **Dashboard Proyeksi** (tab "🚀 Proyeksi" di Wealth → Total, `js/views/wealth.js`
  ...
  **SENGAJA GA ADA garis historis "Nabung doang" terpisah** (sempat ada — `savingsOnlySeries()`
  ...
  (helper `blurTick()`/`blurMoneyTooltip` wealth.js — SEMUA chart di Wealth udah pakai ini, lihat
  bullet blur mode di atas; chart baru WAJIB pasang dua-duanya). Ga ada anotasi garis
...
- **Cek Integritas Data** (Setting → "🩺 Cek Integritas Data", `js/integrity.js`,
  ...
  yang nunjuk akun/kategori/goal/debt/asset yang udah ga ada (transaksi ber-`assetDeleted` =
  asset dihapus lewat app, BUKAN yatim, di-skip), transfer dengan `toAccountId` =
...
- **Efek samping transaksi ber-`debtId`/`assetId`/`attachmentId` DIPUSATKAN sebagai hook di
  `add()`/`patch()`/`remove()` generik** (kenapa hook, bukan manual di tiap sheet: lihat
  `DECISIONS.md`) (db.js): `applyDebtEffect()` (efek debt, arah & sign SELALU dari
  `debtTxDelta()`/`isDebtBorrow()` calc.js — pembayaran turunin outstanding, borrow naikin),
  `applyAssetQtyEffect()` (reverse `assets.quantity`/nilai qtyless/flag redeemed — HANYA di
  `remove()`, karena create selalu lewat sheet yang udah nulis efeknya sendiri dan edit transaksi
  ber-`assetId` ga didukung, lihat bullet `assets`), hapus dokumen `attachments` + transaksi biaya
  (`feeOfTxId`) ikut di `remove()`. Sheet manapun yang hapus transaksi ga perlu tau soal ini.
  **Jalur tulis yang SENGAJA bypass hook** (raw `writeBatch`/`deleteDoc`, atau flag eksplisit):
  0. `add(name, data, {skipDebtEffect:true})` — flag bypass EKSPLISIT, cuma dipakai
     `openNewDebtSheet()` (debt dibuat dengan outstanding final + transaksi borrow-nya; kalau hook
     jalan, dobel). `deleteAssetKeepHistory()` (hapus asset, transaksi ditandai `assetDeleted`)
     juga raw batch — cuma patch flag, ga ada efek yang perlu di-trigger.
...
Pages nge-serve semua file root (`xiesandi.cyou/fintrack/CLAUDE.md` bisa dibuka siapa aja),
jadi data personal (gaji, nama bank, portfolio, hutang) DILARANG ditulis di file ini,
`DECISIONS.md`, `TASKS.md`, atau `README.md`. Angka finansial riil di narasi insiden juga
dibikin generik (X/Y), bukan nominal asli.
