...
biar CLAUDE.md ga makin panjang tiap ada insiden baru.
...
app ini (`xiesandi.cyou`) dan beberapa symbol saham IDX (blue chip umum), semua balik
harga valid.
...
— bukan dari brainstorm/spec baru. Owner nemu Σ baris "Perubahan komposisi" ga match Δ net
worth section 10 yang beneran, selisihnya ≈ 13% dari Δ (angka riil: `tests/calc.test.mjs`).
...
seharusnya KONTRIBUSI NEGATIF ke net worth. Efeknya: laporan Agustus nunjukin "Debt +d" yang
kebaca seolah debt naik itu NAMBAH net worth, dan ke-jumlah sebagai +d alih-alih -d.

**Verifikasi dua bug itu SEKALIGUS jelasin selisihnya** (angka riil diganti simbol — repo
public; fixture aslinya ada di `tests/calc.test.mjs` sebagai regression test): dari data laporan
(Cash −c, Assets(RAW) −a, CAPEX −k, Goal Savings −g, Debt(RAW) +d) — formula BENAR (exclude
baris CAPEX terpisah karena udah nempel di Assets, negasi Debt) ngasih `-c + -a + -g - (+d)`,
PERSIS match Δ net worth beneran. Kode lama nambahin `-k` (CAPEX, double count) DAN pakai `+d`
(bukan `-d`, sign kebalik). Dua bug itu ga saling menutupi, keduanya nyumbang ke selisih (k + 2d) yang
diketemuin owner (dua kesalahan sign/exclude nubruk kebetulan deket, TAPI keduanya harus
dibenerin terpisah — bukan satu fix nutupin dua-duanya).
...
## TASK-3: Utang CC jauh lebih gede dari expense Agustus — diinvestigasi, BUKAN bug (2026-08)

_(Angka riil di entry ini SENGAJA diganti X/Y — repo public, lihat CLAUDE.md "Konteks Owner".
X = total utang dua kartu, Y = total expense Agustus, Y ≈ 17% dari X.)_

**Kecurigaan awal (dari audit data, export Agt 2026):** akun kartu kredit nunjukin utang total
X (dua kartu digabung), tapi total expense tercatat BULAN AGUSTUS (semua akun digabung,
section 2 report) cuma Y. Kecurigaannya: ada belanja
...
1. **Argumen konservasi uang.** Total expense Agustus (SEMUA kategori, SEMUA akun — bukan cuma
   CC) cuma Y. Bahkan kalau 100% expense itu SEMUANYA ke CC (skenario paling ekstrem),
   itu cuma bisa jelasin maksimal Y dari X total utang CC. Sisanya, MINIMAL
   X − Y (≈82,5% dari total utang), SECARA MATEMATIS TIDAK MUNGKIN berasal dari transaksi
   Agustus — harus dari `initialBalance` kedua akun CC saat dibuat. Owner sendiri udah
...
3. **Bukti pendukung dari data asli.** Section 3 report (Expense per Kategori Agt 2026) beneran
   nunjukin baris "⚖️ Penyesuaian Saldo" (nominal kecil) — bukti LANGSUNG bahwa mekanisme reconcile di
   app ini emang lewat transaksi normal (nongol di laporan expense), bukan patch tersembunyi.

**Kesimpulan: BUKAN bug.** Utang CC X legit — mayoritas (≥82,5%, kemungkinan besar
seluruhnya) berasal dari `initialBalance` yang diisi saat kedua akun CC dibuat,
...
Setting → Akun → Edit akun CC-nya, atau share backup JSON lengkap.
...
## TASK-2: Goal savings — "Rp 0" vs "Terkumpul Rp X" kontradiktif (2026-08)

**Ketemu dari:** audit data pemakaian nyata (export .md Agustus 2026, sama kayak TASK-1/TASK-3)
— owner liat goal "Dana Pensiun" (di-fund via reksadana yang di-link, `linkedAssetIds`)
nunjukin dua cerita beda: section 1 bilang "Goal savings: Rp 0", section 8 bilang "Terkumpul
Rp X = topup Rp 0 + asset Rp X". Kelihatan kontradiktif walau dua-duanya SECARA
TEKNIS benar.
...
"sekarang", biar entry lama/auto-post selalu keurut di BAWAH entry baru hari yang sama
(dianggap paling awal hari itu).
...
## Batch bugfix/fitur 2026-09: arsip asset, reconcile minus, transfer lintas kurs, piutang

Empat item dari feedback owner, dikerjain sekaligus. Yang punya keputusan desain (bukan sekadar
fix) dicatat di sini.

**1. Asset kosong ga bisa dihapus → hapus BOLEH, transaksinya tetap.** Asset yang udah dijual
habis tetap punya jejak transaksi ber-`assetId`, dan guard hapus nolak — posisi Rp0 nggantung
selamanya di list Assets. Percobaan pertama: `isArchived` kayak akun/goal (sembunyiin dari list,
bisa di-un-archive). DITOLAK owner — dia mau asset yang emang udah ga dipunya beneran ILANG, bukan
disembunyiin di balik filter; arsip cuma mindahin masalah. Yang dipilih: hapus dokumen asset-nya,
TAPI transaksinya TIDAK ikut dihapus (cascade = riwayat cashflow beli/jual lenyap & saldo akun
berubah diam-diam — itu yang bikin guard lama ada). Transaksi cuma ditandai `assetDeleted` +
`assetSnapshot` (nama/tipe/currency) biar History masih bisa nampilin "Beli: BBCA (asset
dihapus)" dan buka detail read-only-nya. Syaratnya nilai asset udah 0 — kalau masih ada nilai,
hapus = net worth berubah tanpa jurnal, tetap diblok (jual/nol-in dulu). Integrity ga nge-flag
transaksi ini sebagai orphan (ada penandanya), beda dari orphan hasil hapus manual di console.

**2. Reconcile ga bisa minus.** Akar masalahnya `attachThousands()` nge-strip SEMUA non-digit
termasuk tanda minus (bener buat input nominal transaksi, salah buat saldo aktual). Fix: opsi
`allowNegative` (default tetap strip — jangan diubah, nominal transaksi emang ga boleh negatif)
+ tombol ± karena keyboard numerik HP sering ga punya "-". Buat kartu kredit diputuskan input-nya
"tagihan terpakai" (positif = utang), BUKAN saldo signed: semua tampilan CC di app udah pakai
bahasa "Terpakai / Limit / Sisa" (lihat entry "Kartu kredit: dari cash path (v1) ke debt path
(v2)"), nyuruh user ngetik "-500000" di satu sheet doang bakal jadi satu-satunya tempat yang
mikirnya kebalik. Minus di sheet CC = kelebihan bayar (saldo plus), tetap dibolehin.

**3. Transfer lintas mata uang ngredit angka mentah.** `accountBalances()` dulu ngredit akun tujuan
`amount` apa adanya — IDR 1.5jt masuk ke akun USD jadi "$1.500.000". Fix: field `toAmount`
(nominal diterima, currency tujuan) + `fxRate`. Keputusan: `fxRate` SELALU "1 USD = X IDR"
apapun arah transfernya (bukan "rate dari currency asal ke tujuan") — konsisten sama satu-satunya
konsep kurs yang udah ada di app (`effectiveRate()`, kurs manual di Setting), jadi user ga perlu
mikir "ini 0,0000625 atau 16000". Field-nya additive & `null` buat transfer se-currency, jadi
data lama & transfer biasa TIDAK berubah perilaku. Bayar Tagihan CC SENGAJA ga dikasih field kurs
(dibatasin sumber se-currency) — sheet shortcut, kasus lintas currency-nya pakai Transfer biasa.

**4. Piutang.** Diminta "treat as assets yang mengendap" + toggle net worth. Dua opsi model:
(a) collection baru `receivables` (kayak `debts` tapi kebalik) — butuh CRUD, kalkulasi net worth,
snapshot, report, integrity, bulkDelete semua dari nol; (b) tipe asset baru `receivable` yang
REUSE mesin `qtyless` (DIPILIH). Alasannya: perilaku yang dibutuhin ("kasih pinjaman" = uang
keluar dari akun & sisa naik, "terima pembayaran" = uang masuk & sisa turun, reversal pas hapus,
bulk delete, integrity) itu PERSIS mekanisme buy/sell qtyless yang udah ada, ditest, dan
dipusatkan di db.js. Bedanya cuma label + dua field (`debtorName`, `dueDate`) + P&L dipaksa 0 +
toggle. Bikin collection baru = jalur tulis transaksi keempat yang pattern-nya beda (persis yang
dilarang di entry "efek debt/asset dipusatkan sebagai hook").

Toggle-nya **default INCLUDE** — beda dari CAPEX (default exclude). CAPEX default exclude karena
nambah fitur ke user existing ga boleh tiba-tiba ngubah net worth (barang lama yang baru
diklasifikasikan). Piutang beda: transaksi "Kasih Pinjaman" beneran nurunin cash; kalau piutang
ga dihitung, net worth jatuh persis sebesar pinjaman seolah itu expense — padahal cuma pindah
bentuk. Default include = net worth kekal pas minjemin (yang bener secara akuntansi), user yang
mau konservatif ("anggap ga bakal balik") tinggal matiin toggle-nya. Toggle ditaruh di Wealth →
Total bareng toggle CAPEX (bukan Home) — itu satu-satunya tempat definisi net worth diatur,
Home cuma nampilin hasilnya.

**State operasional sekarang & aturan yang WAJIB dipatuhi:** lihat CLAUDE.md bullet `accounts`
(reconcile), `transactions` (transfer lintas mata uang), `assets` ("Hapus asset yang punya
transaksi", tipe `receivable`), `settings/main`, `snapshots`.

---

## Piutang v2: sumber dana wajib, riwayat cicilan, foto bukti via Firestore (2026-09)

**Tab sendiri.** Awalnya piutang nongol di tab Assets (tipe asset biasa). Owner minta tab
"Piutang" sendiri di samping Debt, dan baris terpisah di breakdown net worth — piutang itu
kebalikan hutang ("siapa utang ke gue"), bukan investasi; nyampur sama saham/emas bikin angka
Assets kebaca dobel. Tab Assets & sumtab Assets sekarang EXCLUDE piutang; `totalAssetsIDR()`
tetap termasuk (konsep net worth, baris "🤝 Piutang" dipisah di breakdown).

**Piutang baru = potong akun.** Form generik Tambah Asset bikin piutang TANPA transaksi (uang
"muncul" jadi piutang tanpa keluar dari akun manapun) — salah buat kasus normal. Sekarang tombol
Tambah Piutang punya sheet sendiri yang WAJIB pilih akun sumber dan sekaligus nulis transaksi
`assetDir:"buy"` — mekanismenya sama persis "Kasih Pinjaman" di piutang existing, cuma asset-nya
dibikin dulu. Form generik tetap bisa (posisi lama yang uangnya keluar sebelum pakai app).

**Cicilan = banyak transaksi, tipe tetap transfer.** Owner minta pembayaran bisa dicicil
"bentuknya income". Dicicil emang udah begitu (tiap Terima Pembayaran = satu transaksi `sell`).
Soal "income": DIPUTUSKAN tipe datanya TETAP `transfer`, cuma TAMPILAN di History yang dibikin
kayak income (+ hijau; pinjaman keluar − merah). Kalau dijadiin `income` beneran, uang balik dari
piutang masuk `monthSummary().income` → saving rate & surplus (pace milestone, proyeksi) naik
palsu, padahal pinjaman keluarnya ga pernah kehitung expense (transfer). Kalau mau simetris
(lend = expense, repay = income), itu ngubah budget/spentByCategory/cashflow semua — keputusan
lebih besar, ditunda sampai owner emang mau piutang masuk cashflow.

**Foto bukti: Firestore, bukan Storage.** Firebase Storage ga di-init di app ini, upload-nya ga
ikut offline persistence (app ini offline-first), dan butuh rules + bucket setup terpisah.
Dipilih: kompres di client (canvas → JPEG, resize ≤1280px, turun bertahap sampai ≤ ~450KB) →
data URL disimpan di collection `attachments` TERPISAH. Kenapa terpisah, bukan field di
transaksi: `state.transactions` di-listen SEMUA lewat onSnapshot — blob nempel di situ = tiap
foto ke-load ke memori tiap sesi & snapshot listener makin berat seiring waktu. Di collection
sendiri, foto cuma di-`getDoc` pas detail dibuka (offline cache Firestore tetap nyimpen yang
pernah dibuka). Batas ~450KB (awalnya 700KB, diturunin setelah review 2026-09): limit 1 MiB/doc
Firestore plus foto ikut cache offline Firestore (default 40 MB, LRU) yang bisa ngusir data
transaksi kalau fotonya gede-gede. Cuma dipasang di jalur
piutang (bukti transfer/nota) — bukan fitur umum semua transaksi, biar ga jadi undangan nimbun
blob di Firestore. Kalau nanti butuh foto di banyak transaksi, saat itu baru evaluasi Storage.

**Label UI "Claim".** Owner minta semua teks yang keliatan user (tab, sheet, toast, laporan .md)
pakai "Claim", bukan "Piutang". Nama field/fungsi/komentar di kode & dokumen tetap
`receivable`/piutang — rename identifier cuma nambah diff tanpa manfaat.

**Piutang ga bisa dihapus, cuma arsip (revisi owner).** Buat asset lain, "hapus asset kosong,
transaksi tetap" udah cukup. Buat piutang owner minta sebaliknya: JANGAN pernah bisa dihapus —
rekam jejak siapa pinjam berapa & udah bayar berapa harus tetap keliatan sebagai satu entitas
(bukan cuma transaksi yatam ber-badge "asset dihapus"). Arsip = ditutup: nilai 0 (lunas, atau
write-off kalau ga ketagih — ini bonus, ga perlu mekanisme write-off terpisah), disembunyiin ke
section Arsip. `isArchived` di sini KHUSUS piutang — jangan disamaratakan ke tipe asset lain
(itu persis fitur arsip yang udah dicabut di entry sebelumnya).

**State operasional sekarang & aturan yang WAJIB dipatuhi:** lihat CLAUDE.md bullet `assets`
tipe `receivable` dan `attachments`.

---

## Hutang v2: disamain sama piutang (2026-09)

Owner minta hutang non-CC (`debts`) diperlakukan sama kayak piutang: sumber/tujuan dana, riwayat
pembayaran per hutang, foto/link bukti, ga bisa dihapus cuma arsip. Yang beda dari piutang dan
kenapa:

**Pembayaran TETAP expense ber-`debtId`** (bukan transfer kayak pembayaran piutang). Cicilan
hutang emang udah dari awal dimodelkan sebagai expense (kategori Cicilan/Debt, masuk budget &
cashflow) — owner nge-budget-in cicilan tiap bulan. Ngubah jadi transfer bakal ngilangin cicilan
dari Expense/budget yang udah dipakai setahun. Jadi asimetri sama piutang (yang pakai transfer)
DITERIMA: hutang = "kewajiban yang dibayar dari cashflow", piutang = "asset yang uangnya balik".

**Dana pinjaman masuk = transfer `debtDir:"borrow"`, bukan income.** Uang pinjaman masuk rekening
bukan pendapatan; kalau income, saving rate & surplus (pace milestone, proyeksi) naik palsu. Arah
disimpan eksplisit di transaksi (`debtDir`), data lama tanpa field itu = pembayaran, jadi
perilaku lama ga berubah. Hook debt di db.js sekarang pakai `debtTxDelta()` (calc.js) sebagai
satu-satunya sumber sign — sebelumnya sign di-hardcode `-amount` di tiap hook, nambah arah baru
di tiga tempat + bulkDelete itu resep bug.

**`skipDebtEffect` di `add()`.** Hutang Baru bikin dokumen debt + transaksi borrow sekaligus.
Kalau outstanding dibikin 0 lalu ngandelin hook, hook bisa gagal nemu debt di `state` (snapshot
listener belum emit) → outstanding nyangkut 0. Dipilih: outstanding ditulis langsung = nominal,
transaksi borrow di-`add()` dengan bypass eksplisit — persis pola yang CLAUDE.md wajibkan buat
jalur tulis yang efeknya udah final ("flag semacam skipSideEffects"). Reversal pas hapus tetap
lewat hook `remove()` (bener: hapus borrow = outstanding turun).

**Checkbox "dana masuk ke akun" default ON, tapi bisa OFF.** BNPL/cicilan barang uangnya ga
pernah lewat rekening — hutang ada, tapi ga ada transaksi masuk. Beda dari
